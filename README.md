...
- Messages: encrypted private chat (gift-wrap/NIP-17 flows), with image and PDF attachments. Pasting an image with Cmd+V, Ctrl+V, or the phone's system Paste action, or picking a file with the gallery button, stages it above the message field with a preview; the send button ships the attachment and then any typed text as a second message. A staged file survives navigating away from the chat.
