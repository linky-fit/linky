...
- Wallet: Cashu token ingest, restore, validation, spend; the wallet inventory is one row per proof (available, held by a payment, handed out, spent), with the mint's live answer per proof and the open transfers (issued, pending, failed) listed separately. A token too large for one QR is shown as a NUT-16 animated QR and reassembled by the scanner. Issued tokens nobody claimed can be taken back at once with `Return unclaimed` on the tokens page, or automatically after a chosen wait via the Advanced setting `Return unclaimed tokens` (off by default; the token detail says when it will happen)
