...
- Native Bluetooth discovery: nearby friends grouped in Contacts and nearby users first in Add contact suggestions, with profiles fetched from Nostr while the app is open. The experimental public BitChat room entry is hidden pending UX review. Private chats continue over Nostr. See [device testing and current limits](./docs/bluetooth-testing.md).
...
- Wallet: Cashu token ingest, restore, validation, spend; the wallet inventory is one row per proof (available, held by a payment, handed out, spent), with the mint's live answer per proof and the open transfers (issued, pending, failed) listed separately
