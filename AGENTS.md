...
Before rotating message fixtures, wait for their final publish receipts (`status = sent` and a non-pending `wrapId`) on both devices. A visible bubble can still be pending; a late receipt updates the row and correctly copies it into the active shard.
...
The compose image is built with `VITE_E2E=1`, which makes `main.tsx` install `window.__linkyE2E` (`src/devtools/e2e/installLinkyE2eHooks.ts`): raw Evolu upserts under any owner, `useOwners` for legacy lane mnemonics, `shardRows(scope, table)` for what the linksync shards hold, `ownerProofRows(ownerId)` for legacy-copy assertions, `shardOwnerId(scope, index)`, `syncOwnerIds()` and `forget()`. `lane-migration.spec.ts` seeds legacy lane rows through it instead of the UI, so it keeps working after the app stops writing to the lanes. When building the image by hand, pass `--build-arg VITE_E2E=1`; production builds never set it.
...
- During the lane migration grace period, `useLaneToShardMigration` mirrors only terminal `cashuProof.state = spent` to existing legacy proof rows, so older clients stop counting spent funds. This is the sole legacy write exception; never copy new proofs or spendable states back. Remove it with the lane migration under the architecture removal gate.
...
- `@evolu/sqlite-wasm` is patched to preserve OPFS files when pool initialization fails and settle all handle acquisitions before cleanup. Keep all three JS entry bundles patched when upgrading; run the SQLite lock-failure cases in `tests/boot-recovery.spec.ts` and the quota-recovery E2E test.
