...
A keyset id is bound to the mint that owns it (stored under `linkshu.keysetMint.<id>`). Ownership is established from existing holdings first (a proof already stored under the keyset), then from the stored binding, so an impostor cannot claim a keyset the wallet already holds proofs under — even after an upgrade with an empty binding store or a fresh device sync. Loading a mint whose active keyset id belongs to a different mint fails with `MintRejected` on every load, cached included; the check-and-write runs under a per-keyset lease so two concurrent loads cannot both claim an unbound keyset, and a mint with no active keyset (restore-only) is skipped, not crashed. A keyset id is a hash of the keyset's public keys (NUT-02), so honest mints never collide; the check stops a mint from impersonating another mint's keyset to force NUT-13 counter reuse. Only when there are neither holdings nor a stored binding does first-load win, so a brand-new keyset an impostor presents before the honest mint (and before any holdings) is a denial of service on that mint, never fund loss.
