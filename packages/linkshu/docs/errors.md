...
| `AmountConsumedByFee`       | `mint`, `amount`, `fee`                    | The mint's input fee to swap the proofs (NUT-02) is at least their value: such a token cannot be redeemed on its own, and a send of `amount` would issue one. Nothing was stored; the mint was not called. | Tell the user the amount has to exceed the mint's fee `fee`.                                                                                               |
