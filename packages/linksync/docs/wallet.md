...
A new proof insert returns the persisted row's creation time. It does not sample the clock before the write, which can cross a second boundary; inserting the same secret again returns the same creation time.
